...
def get_channel_accounts(login, request_data):
    global request_url
    request_url = login.get_base_url() + request_data['path']
    channel_accounts_response = requests.get(url=request_url, headers=login.get_header_get_request())
...
def test_delete_channel_account(login, request_data):
    ...
    for account in channel_accounts_response_dict:
        if account['appId'] == request_data['requestBody']['appId'] \
                and account['originalAccountId'] == request_data['requestBody']['originalAccountId'] and \
                account['name'] == request_data['requestBody']['name']:
